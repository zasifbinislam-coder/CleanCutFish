...
const STATUS_LABEL = {
  received: { bn: "গৃহীত", en: "Received", note: "Your order is in our queue." },
  confirmed: { bn: "নিশ্চিত", en: "Confirmed", note: "We've confirmed your order and started preparing it." },
  shipped: { bn: "পথে আছে", en: "Out for delivery", note: "Your order is on the way." },
  delivered: { bn: "পৌঁছেছে", en: "Delivered", note: "Hope you enjoy the catch — please leave a review when you can." },
  cancelled: { bn: "বাতিল", en: "Cancelled", note: "If this wasn't expected, reply to this email and we'll sort it out." },
};

export function orderStatusEmail(order, status) {
  const s = STATUS_LABEL[status] ?? { bn: status, en: status, note: "" };
  return wrap(`
    <p>আসসালামু আলাইকুম <strong>${escapeHtml(order.name)}</strong>,</p>
    <p>অর্ডার <code style="background:#f4f4f4;padding:2px 6px;border-radius:4px">${escapeHtml(order.id)}</code>
       বর্তমান অবস্থা: <strong>${s.bn}</strong>.</p>
    ${s.note ? `<p style="color:#555">${escapeHtml(s.note)}</p>` : ""}
    <hr style="border:none;border-top:1px solid #eee;margin:16px 0"/>
    <p style="font-size:13px;color:#555"><em>English:</em> Your order
      <strong>${escapeHtml(order.id)}</strong> is now <strong>${s.en}</strong>.${s.note ? ` ${escapeHtml(s.note)}` : ""}</p>
  `);
}
